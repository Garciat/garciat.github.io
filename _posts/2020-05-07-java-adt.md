...
I find Algebraic Data Types (ADTs) to be a powerful domain modeling tool. (TODO: explain why :)
...
Indeed, `new Cursor.Start()` is-a `Cursor`, as are `new Cursor.Next("magic value in here")` aand `new Cursor.End()`.
...
We still need to handle with that pesky `IllegalArgumentException`, though. Why?
