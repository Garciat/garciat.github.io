...
In abstract syntax: `Background = Transparent + SolidColor + Image`. You can read the `+` as "or". That is: "a `Background` is either `Transparent` **or** a `SolidColor` **or** an `Image`."
