...
as parameters instead of reading them from `globalThis`; and I `export class Go`
instead of `globalThis.Go = class Go`. Sadly, I am not able to change Go's
dependency on `globalThis.fs` because that happens in the standard library.

Finally, I renamed it to `./wasm_exec@1.23.4.ts` in order to make it clear that
it is tied to Go version 1.23.4.
